# SPEC_USER §7 — Question Selection System (draft v3 for v3.2)

> **Status:** DRAFT v3 2026-05-21 — locked content pending final Bui sign-off
> **Replaces:** §7 entirety in SPEC_USER_v3.1.md "Smart Question Selection" (4-pool priority becomes §7.4 nested). §6 Bible Journey Map LOCKED status removed (see §7.10 cross-ref).
...
> - v2: 11 fixes (B1-B2 PK convention, I1-I4 inconsistencies, G1-G5 gaps)
> - v3 (this): 7 nits (N1 season_id BIGINT→VARCHAR(36) verified via Season.java, N2 INSERT id column, N3 MySQL multi-drop syntax, N4 current_difficulty timing split, N5 throttle coverage_book_ticked event, N6 FK/index pre-check before rename, N7 resolve §7.14 open questions)
...
| `seasonId` | `season_id` | `String(36)` | FK → `seasons.id` (verified UUID v4 from Season entity, `apps/api/src/main/java/com/biblequiz/modules/season/entity/Season.java`) |
...
| `coverage_book_ticked` | `{ book, count_at_tick, tier, week }` | **Chỉ fire trên transition `count: 3 → 4`** (sách vừa cross coverage threshold). KHÔNG fire mỗi câu trả lời. Giảm event volume ~10x. |
