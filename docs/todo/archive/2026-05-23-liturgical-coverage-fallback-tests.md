...
- LCT-1 BE: fallback branch 1 — drop same-day exclusion
  - Status: [x] DONE · Files: `apps/api/.../RankedControllerTest.java` or
...
- LCT-4 BE: `/coverage/unlock-next-week` cooldown + error codes
  - Status: [x] DONE · Files: `LiturgicalCoverageServiceTest` or controller
    test
...
- LCT-5 BE: season transition boundary
  - Status: [x] DONE · Files: `LiturgicalSeasonServiceTest`
...
- LCT-6 BE: ×1.5 in-season scoring multiplier
  - Status: [x] DONE — covered by SCD-2 test
    `calculateWithTier_inSeasonBook_appliesAdditional1_5x` (2026-05-23 commit
    d5ebe057). Files: `ScoringServiceTest`
...
- LCT-7 BE: coverage badge award + `noti_24h_sent_at`
  - Status: [x] DONE · Files: `AchievementServiceTest` or coverage badge
    service test
