...
pairs = json.load(open("scripts/seed/label_strip_pairs.json", encoding="utf-8"))
...
lines.append(f"SELECT CONCAT('target rows present: ', COUNT(*)) FROM questions "
             f"WHERE source='seed:json' AND language='vi' AND content IN ({contents});")
...
for p in pairs:
    ...
    lines.append(f"UPDATE questions SET content={q(p['new'])}, updated_at=NOW() "
                 f"WHERE source='seed:json' AND language='vi' AND book={q(p['book'])} "
...
Path("scripts/seed/prod_label_strip.sql").write_text("\n".join(lines) + "\n", encoding="utf-8")
print(f"-> scripts/seed/prod_label_strip.sql ({action}), {len(pairs)} updates")
