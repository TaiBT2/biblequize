...
export default function QuestionQueue({ data }: { data: QueueData | null }) {
  const items = [
    { label: 'Pending Review', value: data?.pendingReview ?? 0, color: 'bg-[#e8a832]', max: 500 },
    { label: 'AI Generated', value: data?.aiGenerated ?? 0, color: 'bg-blue-500', max: 2000 },
    { label: 'Community', value: data?.communitySubmissions ?? 0, color: 'bg-green-500', max: 200 },
  ]
  ...
  return (
    <div className="bg-[#1d1f29] p-6 rounded-lg">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h3 className="text-[11px] uppercase tracking-[0.2em] text-[#d5c4af]/60 font-semibold">Question Queue</h3>
      </div>
