...
export default function RankingsAdmin() {
  ...
  return (
    <div className="space-y-6">
      ...
      {showCreate && (
        <div className="rounded-lg border border-[#504535]/20 bg-[#1d1f29] p-4 space-y-3">
          <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Tên mùa giải" className="w-full bg-[#191b25] border border-[#504535]/20 rounded-lg px-4 py-2 text-sm text-[#e1e1ef]" />
          ...
          <button onClick={createSeason} disabled={isSaving} className="px-6 py-2 gold-gradient text-[#281900] rounded-lg text-sm font-bold disabled:opacity-50">{isSaving ? 'Đang tạo...' : 'Tạo mùa giải'}</button>
        </div>
      )}
      ...
      {active && (
        <div className="rounded-lg border-2 border-emerald-500/30 bg-emerald-500/5 p-5">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => endSeason(active.id)} className="px-4 py-2 bg-red-600/20 text-red-400 rounded-lg text-sm font-medium hover:bg-red-600/30 border border-red-500/20">Kết thúc sớm</button>
          </div>
        </div>
      )}
      ...
      {isLoading ? <div className="text-center text-[#d5c4af]/60 py-8">Đang tải...</div>
       ...
       : <div className="space-y-3">{seasons.filter(s => !s.isActive).map(s => (
