...
- `[OBS]` **Revert về PENDING** (`PUT /questions/{id}` đổi `reviewStatus` ACTIVE/REJECTED → PENDING): mở **chu kỳ duyệt mới** — reset `approvalsCount=0` + xoá toàn bộ `QuestionReview` của câu (`reviewRepository.deleteByQuestionId`). Nếu bỏ bước này, review của người duyệt cũ vẫn loại câu khỏi `pendingForMe` của mọi admin (vô hình trong queue dù vẫn `totalPending`). Method `update` là `@Transactional` (ghi 2 bảng); safeguard Bible Basics ném lỗi → rollback cả xoá review.
