package com.biblequiz.modules.adminai.provider;

import com.biblequiz.modules.adminai.AIGenerationService;
import org.springframework.stereotype.Component;

import java.util.List;
import java.util.Map;

/** Thin adapter delegating to existing {@link AIGenerationService#generate}. */
@Component
public class GeminiProvider implements AIProvider {

    private final AIGenerationService delegate;

    public GeminiProvider(AIGenerationService delegate) {
        this.delegate = delegate;
    }

    @Override
    public String getProviderName() { return "gemini"; }

    @Override
    public boolean isAvailable() { return delegate.isConfigured(); }

    @Override
    public AIGenerationResult generate(AIGenerationContext ctx) {
        try {
            List<Map<String, Object>> questions = delegate.generate(
                    ctx.book(), ctx.chapter(), ctx.verseStart(), ctx.verseEnd(),
                    ctx.difficulty(), ctx.type(), ctx.language(), ctx.count(),
                    ctx.scriptureText(), ctx.customPrompt());
            return AIGenerationResult.of(questions, getProviderName());
        } catch (Exception e) {
            throw new AIProviderException("Gemini failed: " + e.getMessage(), e);
        }
    }
}
